  ...
  return withTenant(user.tenantId, user.id, async db => {
    ...
    if (can("intake:read")) {
      ...
      const stages = (await db.query(
        ...
        [user.facilityIds])).rows as { stage: string; count: number }[];
      const byStage = Object.fromEntries(stages.map(row => [row.stage, row.count]));
      result.intake = {
        appointments: appointments.total, checkedIn: appointments.checked_in,
        waiting: byStage.waiting ?? 0, workup: byStage.workup ?? 0,
        dilation: byStage.dilation ?? 0, consultation: byStage.consultation ?? 0,
      };
    }
    ...
    if (can("clinical:read")) {
      const row = (await db.query(`SELECT
        ...
        [user.facilityIds, user.id, today])).rows[0];
      result.clinical = { open: row.open, mine: row.mine, drafts: row.drafts, signedToday: row.signed_today };
    }

    if (can("prescription:read")) {
      const row = (await db.query(
        "SELECT count(*)::int AS signed FROM app.prescription WHERE status='signed'")).rows[0];
      // "Awaiting dispensing" is only meaningful to someone who can dispense.
      const awaiting = can("pharmacy:dispense")
        ? (await db.query(`SELECT count(*)::int AS pending FROM app.prescription r
        ...
        : null;
      result.prescriptions = { signed: row.signed, awaitingDispense: awaiting };
    }

    if (can("preview:inventory")) {
      const row = (await db.query(`WITH b AS (
        ...
        SELECT
          coalesce(sum(b.quantity * b.unit_cost_paisa), 0)::float8 AS stock_value,
          ...
          (SELECT count(*)::int FROM app.stock_threshold t
             WHERE t.facility_id = ANY($1::uuid[])
        ...
        FROM b`, [user.facilityIds, today, horizon])).rows[0];
      result.inventory = {
        stockValue: row.stock_value, lowStock: row.low_stock,
        expiringSoon: row.expiring_soon, quarantined: row.quarantined,
      };
    }
    ...
    if (can("preview:billing")) {
      const row = (await db.query(`SELECT
        ...
        FROM app.invoice i WHERE i.facility_id = ANY($1::uuid[])`, [user.facilityIds, today])).rows[0];
      const collected = (await db.query(
        ...
        [user.facilityIds, today])).rows[0];
      const session = (await db.query(
        "SELECT 1 FROM app.cashier_session WHERE cashier_id=$1 AND closed_at IS NULL LIMIT 1", [user.id])).rowCount;
      result.billing = {
        outstanding: row.outstanding, collectedToday: collected.total,
        invoicesToday: row.invoices_today, sessionOpen: !!session,
      };
    }

    if (can("admin:read")) {
      const row = (await db.query(`SELECT
          (SELECT count(*)::int FROM app.user_account WHERE status='active') AS active_staff,
          (SELECT count(*)::int FROM app.facility WHERE active) AS facilities`)).rows[0];
      const auditToday = can("audit:read")
        ? (await db.query("SELECT count(*)::int AS total FROM app.audit_log WHERE at >= $1::date", [today])).rows[0].total
        : null;
      result.governance = { activeStaff: row.active_staff, facilities: row.facilities, auditToday };
    }
